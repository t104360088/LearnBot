 ...
 */
public class Eliza implements Talker {
    /**
     * The key list
     */
    ...
    public String getInitial() {
        return initial.get(new Random().nextInt(initial.size()));
    }

    public Eliza(InputStream is) {
    ...
    private String sentence(String s) {
        ...
        if (quit.find(s)) {
            finished = true;
            return finl.get(new Random().nextInt(finl.size()));
        }
    ...
    private String assemble(Decomp d, String[] reply, Key gotoKey) {
        ...
        while (EString.match(rule, "* (#)*", lines)) {
            ...
            work.append(lines[0]).append(" ").append(reply[n]);
        }
        work.append(rule);
        if (d.mem()) {
            mem.save(work.toString());
            return null;
        }
